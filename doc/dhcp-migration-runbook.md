...
## 8. Пул исчерпан → аренда 12ч→3ч (2026-07-18)

**Симптом:** часть WiFi-клиентов не получала IP. **Причина:** пул 10.11.5.0/24 (`.50–.255` = 206
адресов) забит — 194/206 занято, но **активных лишь ~34, а ~160 протухших**: аренда была **12ч**
(`43200s`), ушедшие устройства держали IP полдня. В `ratzek-dnsmasq.log` — 120k+ строк
`no address available`.

**Фикс (host-direct, ansible устарел):**
```
cp -a /etc/ratzek-dnsmasq.conf /etc/ratzek-dnsmasq.conf.bak-$(date +%F-%H%M%S)
sed -i 's/,43200s/,10800s/g' /etc/ratzek-dnsmasq.conf     # обе dhcp-range → 3ч
dnsmasq --test -C /etc/ratzek-dnsmasq.conf                 # syntax OK
systemctl restart dnsmasq-ratzek                           # NB: SIGHUP/reload НЕ меняет lease-time
                                                           #     для dhcp-range нужен restart
```
Проверка: свежая выдача должна давать `option: 51 lease-time 3h` (в логе); аренды из lease-файла
переживают restart.

**Немедленная чистка протухших** (когда нет `dhcp_release`/dnsmasq-utils): снять активные IP из
ARP, стоп → оставить в lease-файле только активные → старт:
```
ip neigh show dev eth0 | awk '/REACHABLE|STALE|DELAY|PROBE/{print $1}' | grep '^10\.11\.5\.' | sort -u >/tmp/a
cp -a /var/lib/misc/dnsmasq.leases{,.bak-$(date +%F-%H%M%S)}
systemctl stop dnsmasq-ratzek
awk 'NR==FNR{a[$1]=1;next} ($3 !~ /^10\.11\.5\./)||($3 in a)' /tmp/a /var/lib/misc/dnsmasq.leases >/tmp/l && mv /tmp/l /var/lib/misc/dnsmasq.leases
systemctl start dnsmasq-ratzek        # активные клиенты не затронуты; ушедшие перезапросят IP
```
Результат: 194→33 аренд, 173 свободных, выдачи пошли. Ёмкости хватает (реальных ~34) — вторая
подсеть (10.11.4) не нужна.

**NB — DHCP-флуд «Commutator»:** MikroTik-свитч «Commutator» (`10.11.5.158`, MAC
`18:fd:74:d9:71:fd`, в mikrotik-exporter НЕ мониторится) шлёт ~1.7 DISCOVER/с с полностью
обнулённым L2 (ethernet-src/chaddr/client-id = 0), `secs`~18000 (застрял на часы). Лизы НЕ ест
(никогда не ACK'ает), но засоряет лог. Лечится на самом свитче: убрать/починить кривой
`/ip dhcp-client` (вероятно на bridge/VLAN-интерфейсе без MAC).
