...
- **None-learn наследование — закрыто в 0.1.39** (было главным остаточным: `(None→new)` с живым чужим
  счётчиком молча учился без сброса; ловил только 3ч-окно). Теперь сбрасывается при счётчике `> quota/4`.
...
- **Массовый ложный un-throttle тяжёлых при bare-restart + двойном IO-отказе.** Если при голом рестарте
  процесса owner-файл И leases одновременно нечитаемы (флаки USB-SSD), все члены сеются `mac=None`; на
  восстановлении лиз каждый с счётчиком `> quota/4` сбросится пачкой. Un-throttle-safe, self-heal. **Ребут
  по питанию обнуляет ipset (счётчики 0 → сбрасывать нечего)** → касается только редкого bare-restart.
  Ловится алертом `RatzekShaperQuotaInheritanceResetSpike`.
